...
class ReleaseConfigTests(unittest.TestCase):
    ...
    def test_patch_manifest_contains_locale_allowlist_and_runtime_fallback_rules(self) -> None:
        ...
        self.assertTrue(locale_patch["critical"])
        self.assertIn('"zh-CN"', locale_patch["replace"])
        self.assertIn('Yk=["en-US"', locale_patch["find"])
        self.assertIn('"id-ID"]', locale_patch["find"])
        self.assertIn("regexFind", locale_patch)
        self.assertIn("regexReplace", locale_patch)
        self.assertIn("postCheck", locale_patch)
        self.assertIn("(?<default>", locale_patch["regexFind"])
        self.assertIn('${list}=["en-US"', locale_patch["regexReplace"])

        language_name_patch = next(
            item for item in patches if item["description"] == "语言菜单显示简体中文"
        )
        self.assertTrue(language_name_patch["critical"])
        ...
        self.assertIn("getDisplayNames(Zk,Rts)", language_name_patch["find"])
        self.assertIn('name:"zh-CN"===s?"简体中文"', language_name_patch["replace"])
        self.assertIn("localName:e.formatters.getDisplayNames(s,Rts).of(s)", language_name_patch["find"])
        self.assertIn("regexFind", language_name_patch)
        self.assertIn("regexReplace", language_name_patch)
        self.assertIn("postCheck", language_name_patch)
        self.assertIn("(?<display>", language_name_patch["regexFind"])
        self.assertIn('name:"zh-CN"===${item}?"简体中文"', language_name_patch["regexReplace"])
